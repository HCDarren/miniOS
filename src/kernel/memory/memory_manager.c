...
// 之前 0x1000 那个 loader 区域我们现在反正不用了，就用来存页表的信息
// 内核页目录索引
#define KERNEL_PAGE_DIR 0x1000
// 内核页表索引
static u32_t KERNEL_PAGE_TABLE[] = {
    0x2000,
    0x3000,
};
...
static void create_memory_mapping(page_mapping_dir_t *page_dir, void *virtual_addr, void *physics_addr, u32_t count)
{
    ...
    {
        /*page_mapping_table_t *page_mapping_table = find_page_mapping_table(page_dir, virtual_addr, 1);
        if (page_mapping_table == NULL)
        {
            return;
        }
        assert(page_mapping_table->present == 0);
        page_mapping_table->phy_page_addr = (u32_t)physics_addr;
